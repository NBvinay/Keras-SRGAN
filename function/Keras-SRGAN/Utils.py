...
def load_test_data(directory, ext, number_of_images = 100):
    ...
    image=cv2.imread('F:\wamp\www\superres\\function\Keras-SRGAN\data_lr\input_image.jpg')
    newImg = imresize(image, (96,96) , interp = 'bicubic', mode = None)
    cv2.imwrite('F:\wamp\www\superres\\function\Keras-SRGAN\data_lr\input_image.jpg',newImg)
...
def plot_test_generated_images(output_dir, generator, x_test_lr, figsize=(5, 5)):
    ...
    for index in range(examples):
        ...
        plt.imshow(generated_image[index], interpolation="bicubic")
        plt.axis('off')
        
        plt.tight_layout()

        plt.savefig('F:\wamp\www\superres\\function\Keras-SRGAN\output\sr_image.jpg',bbox_inches='tight',pad_inches = 0)
        print('saving images')
        ...
        img = cv2.imread('F:\wamp\www\superres\\function\Keras-SRGAN\output\sr_image.jpg')

        dst = cv2.fastNlMeansDenoisingColored(img,None,4,4,7,21)

        cv2.imwrite('F:\wamp\www\superres\\function\Keras-SRGAN\output\sr_image_smooth.jpg',dst)
